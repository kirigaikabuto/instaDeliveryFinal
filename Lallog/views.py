...
def kalkul(request):
    ...
    context = {'mydate': mydate, 'mydate1': mydate1, 'mydate2': mydate2, 'mydate3': mydate3, 'mydate4': mydate4,
               'mydate5': mydate5}
...
def get_calcul(request):
    ...
    order.save()
    text = "Новый заказ:" + " " + str(
        order.client) + ", " + "Откуда:" + " " + order.from_address + " " + "Куда:" + " " + order.to + ", " + "Доставить до:" + " " + order.to_date_until + ", " + str(
        order.curier_money) + " " + "Тенге курьеру" + ", " + str(order.ves) + "Кг."
...
def status_change(request):
    ...
    if current_test_order.status == "Забрал":
        change = Changes.objects.create(user=request.user.mycurier, balance_before=request.user.mycurier.balance - (
                (current_test_order.itog * 35) // 100), summa=((current_test_order.itog * 35) // 100),
                                        reason="Забрал заказ")
        change.save()
...
def all_curier_data(request):
    ...
    for i in orders:
        obj = {
            ...
            "Номер Клиента": i.from_phone,
            "Куда": i.to.split(",")[0],
            "Номер Доставки": i.to_phone,
            "Дата": i.to_date,
            "доставить до": i.to_date_until,
            "Забрать Наличные": i.nal,
            "Вес": i.ves,
            "Статус": i.status
        }
